...
| 2026-09-26 (T113) | `RulegateErrorCode` gains `'W_INTEROP_OUTPUT_LEFT'`                                                                                 | Another tool's generated file that `init` masked from the import, because its source came across instead, and that no enabled adapter renders back to the same path. Nothing after `init` can see it — it is not imported and not in `state.json` — so `init` names it. A union member added to an exported type widens what an adapter may receive and never what it must produce, so no adapter breaks; one that switches exhaustively over the codes gains a case to handle.                   |
