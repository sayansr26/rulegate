...
| `opencode`    | `.opencode/opencode.json`, `.opencode/rules/*.md`                           |
...
**`CLAUDE.md`** — repo-wide rules, concatenated:
...
**`.claude/rules/30-frontend.md`** — one file per glob-scoped rule, because Claude Code scopes
natively with `paths:`:
...
`paths` is the only key Claude Code reads, so the description is a body heading; every glob is
double-quoted because an unquoted `*` in a list item makes the frontmatter unparseable, and
Claude Code then loads the rule repo-wide. Tools with no per-path mechanism (Codex, Gemini,
Cline) still state the scope in a prose `**Applies to:**` line — a documented lossy mapping.
